...
import { REPORT_TIME_RANGE } from "@src/enums";
import DashboardService from "@src/services/DashboardService";
import MediaService from "@src/services/MediaService";
import { formatDay, formatPrice, generateKey, randomHexColor } from "@src/utils";
import { isEmpty } from "lodash";
import { BarChart3, Bookmark, Calendar, ChartBarIncreasing, ClipboardList, DollarSign, PieChart as PieChartIcon, RotateCw, Star, Users } from "lucide-react";
...
const AdminReportsPage = () => {
    ...
    const [time, setTime] = useState(REPORT_TIME_RANGE.DAILY.value);
    const [loading, setLoading] = useState(true);
    const [data, setData] = useState({});

    const fetchData = useCallback(
        async () => {
            const { success, payload } = await DashboardService.getReports({ timeRange: time });
            if (success) {
                setLoading(false);
                setData(payload);
            }
        ...
        return (
            <div className="w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    {[...Array(4)].map((_, idx) => (
                        <div key={idx} className="animate-pulse rounded-2xl border shadow-sm bg-white p-6 flex flex-col gap-2">
    ...
    const { revenue, booking, bookingStatus, customer, rate, revenueCharts, bookingCharts, revenueByService, lastBooking } = data

    console.log(
        [
            { name: "Đã huỷ", value: bookingStatus.CANCELLED, color: "#f87171" },
            { name: "Hoàn tất", value: bookingStatus.COMPLETED, color: "#4ade80" },
            { name: "Chờ xử lý", value: bookingStatus.PENDING, color: "#fbbf24" },
            { name: "Thành công", value: bookingStatus.SUCCESS, color: "#60a5fa" },
        ]
    );
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 mb-1">Báo Cáo & Thống Kê</h1>
                    <p className="text-gray-500">Phân tích doanh thu và hiệu quả kinh doanh</p>
                </div>
                <div className="flex items-center gap-3 mt-4 md:mt-0">
                    ...
                    >
                        {
                            Object.values(REPORT_TIME_RANGE).map(
                                (item) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div className="rounded-2xl border shadow-sm bg-white p-6 flex flex-col gap-2">
                    <div className="flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div className="rounded-2xl flex flex-col justify-center  border shadow-sm bg-white p-6">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 font-semibold text-gray-900">
                            <PieChartIcon className="w-5 h-5 text-[#FF8800]" />
                            Trạng Thái
                        </div>
                    </div>
                    {
                        Object.values(bookingStatus).every(value => value === 0) ? (
                            ...
                            <PieChart className="mx-auto" width={400} height={400}>
                                <Pie
                                    ...
                                    data={
                                        [
                                            { name: "Đã huỷ", value: bookingStatus.CANCELLED, },
                                            { name: "Hoàn tất", value: bookingStatus.COMPLETED },
                                            { name: "Chờ xử lý", value: bookingStatus.PENDING, },
                                            { name: "Thành công", value: bookingStatus.SUCCESS },
                                        ]
                                    }
                                    innerRadius="0%"
                                ...
                                >
                                    {
                                        [
                                            { name: "Đã huỷ", color: "#e20708" },
                                            { name: "Hoàn tất", color: "#016e2b" },
                                            { name: "Chờ xử lý", color: "#f68e2f" },
                                            { name: "Thành công", color: "#49007e" },
                                        ].map(
                                            ({ color }, index) => (
                                                <Cell key={index} className="" fill={color} />
                                            )
                                        )
                                    }
                                </Pie>
                ...
                <div className="rounded-2xl border shadow-sm bg-white p-6 flex flex-col gap-4">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 font-semibold text-gray-900">
                ...
                <div className="rounded-2xl border shadow-sm bg-white p-6 flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                        <ChartBarIncreasing className="w-6 h-6 text-[#FF8800]" />
                    ...
                    <div className="border-t pt-5 flex items-end justify-between">
                        <div>
                ...
                <div className="rounded-2xl border shadow-sm bg-white p-6 flex flex-col gap-4">
                            ...
                            isEmpty(rate.top) ? (
                                ...
                                (item) => (
                                    <div key={generateKey()} className="flex items-center gap-3 p-3 rounded-xl border border-gray-50 bg-gray-50">
                                        <img className="w-10 h-10 rounded-full object-cover" alt={item.meta.service.name} src={MediaService.getMedia(item.meta.service.image)} />
                                        <div className="flex-1 min-w-0">
                                            <div className="font-semibold text-gray-900 truncate">{item.meta.customer.name}</div>
                                            <div className="text-xs text-gray-500">{item.meta.comment}</div>
                                        </div>
                                        <div className="text-sm font-semibold text-gray-900">{Number(item.meta.rating).toFixed(1)} ⭐</div>
                                    </div>
                                )
                            )
                        }
                    ...
                    <div className="flex justify-between items-center mt-2">
                        <span className="text-gray-500 text-sm">Danh sách đánh giá</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="rounded-2xl border shadow-sm bg-white p-6 flex flex-col gap-4">
                    <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2 font-semibold text-gray-900">
                            <Bookmark className="w-5 h-5 text-[#FACC15]" />
                            Dịch Vụ Hàng Đầu
                        </div>
                    </div>
                    <div className="flex flex-col gap-3">
