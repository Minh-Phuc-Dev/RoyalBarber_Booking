...
import { Calendar, CircleCheck, Clock, Filter, Plus, Scissors, Search, Star, X } from "lucide-react";
...
const bookingSchema = Yup.object().shape(
    {
        ...
        staffId: Yup.string().required("Nhân viên là bắt buộc"),
...
const CreateAppointment = ({ onSuccess }) => {
    ...
    const user = staff.find(
        (staff) => staff.id === Number.parseInt(watch("staffId"))
    )
                                ...
                                >
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                        <div className="space-y-1">
                                            ...
                                            {errors.staffId && (
                                                <p className="text-red-500 text-xs mt-1">{errors.staffId.message}</p>
                                            )}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="block text-sm font-medium text-gray-700 ">
...
const AdminAppointmentsPage = () => {
    ...
    const { bookings: data, loading, error, fetch } = useBookings()
    ...
    const statistics = [
        {
            label: "Tổng lịch hẹn",
            value: data.filter(booking => [BOOKING_STATUS.SUCCESS.value, BOOKING_STATUS.COMPLETED.value].includes(booking.status)).length,
            icon: <Calendar className="w-6 h-6 text-[#1D9BF0]" />,
            iconBg: "bg-blue-50",
        },
        {
            label: "Sắp tới",
            value: data.filter(booking => booking.status === BOOKING_STATUS.SUCCESS.value).length,
            icon: <Clock className="w-6 h-6 text-[#22C55E]" />,
            iconBg: "bg-green-50",
        },
        {
            label: "Đã hoàn thành",
            value: data.filter(booking => booking.status === BOOKING_STATUS.COMPLETED.value).length,
            icon: <CircleCheck className="w-6 h-6 text-[#A855F7]" />,
            iconBg: "bg-purple-50",
        },
        {
            label: "Doanh thu",
            value: formatPrice(
                data.filter(
                    booking => booking.status === BOOKING_STATUS.COMPLETED.value
                ).reduce(
                    (sum, booking) => sum + Number(booking.price),
                    0
                )
            ),
            icon: <Scissors className="w-6 h-6 text-[#FF8800]" />,
            iconBg: "bg-orange-50",
        },
    ];
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {
                    statistics.map(
                        (statistic, index) => (
                            ...
                            >
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${statistic.iconBg}`}>
                                    {statistic.icon}
                                </div>
                                <div>
                                    <div className="text-xl font-extrabold text-[#1A2233] mb-1">{statistic.value}</div>
                                    <div className="text-sm font-medium text-gray-500">{statistic.label}</div>
                                </div>
                            </div>
                        )
                    )
