...
function Dashboard() {
    ...
    const fetchData = useCallback(
        async () => {
            const { success, payload } = await DashboardService.getStatistics();
            if (success) {
                setLoading(false);
                setData(payload);
            }
    ...
    return (
        <div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                ...
                <div className="flex items-center gap-3 mt-4 md:mt-0">
                    <select className="border rounded-lg px-3 py-2 text-gray-700 focus:ring-2 focus:ring-[#FF8800]">
                                ...
                                (item) => {
                                    ...
                                    return (
                                        <div key={item.label} className="flex items-center gap-3 w-full">
                                            ...
                                            <span className="w-12 text-gray-500 text-xs text-right">{item.value}%</span>
                            ...
                            revenueByService.map(
                                (item) => (
                                    <div key={generateKey()} className="flex items-center gap-3 p-3 rounded-xl border border-gray-50 bg-gray-50">
                                        <div className="w-10 h-10 rounded-full flex items-center justify-center bg-white border border-gray-100">

                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="font-semibold text-gray-900 truncate">{item.label}</div>
                                            <div className="text-xs text-gray-500">{Number(item.value).toFixed(0)} lượt đặt</div>
                                        </div>
